...
def draw_bar_cmp(x: list, y_list: list, label_list: list, color_list: list,
              yticks_labels: list, xticks_label: list,
              marker_list: list, xlimit, ylimit, xlabel: str, ylabel: str,
              figsize: tuple, title: str,
              save_name: str, no: int,
              sci=False, log=False,ncols=1,istitle=True):
    # assert len(x) == len(xticks_label)
    print(len(y_list))
    ...
    # plot 曲线
    total_width=0.5
    width = total_width / len(y_list)
    # width = 0.4
    print(x)
    for i, y in enumerate(y_list):
        # plt.bar(x+i*width, y, width=0.4, zorder=10, label=label_list[i],)
        plt.bar([a+i*width for a in x], y, width=width,align='edge', zorder=10, label=label_list[i],)
        # plt.plot(x, y, color=color_list[i], label=label_list[i], marker=marker_list[i])

    plt.yticks(fontproperties='Times New Roman', size=18, ha="right")
    plt.xticks(ticks=np.linspace(x[0]+total_width/2, x[-1]+total_width/2, x_size), labels=xticks_label, fontproperties='Times New Roman', size=18)
